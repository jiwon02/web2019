...
/*
	...
	if(n>=$arr_pic.length-1){  //3>=3
		n=-1;  //다음번에 클릭 했을 대 n=0 값으로 가져와서 배열 데이터의 첫번째 인덱스로 접근해서 호출해야하기 때문
	}*/
} 
...
var $final_img=importImg("<img src='img/tree-1.jpg' alt=''>");  //1번,  6번  
...
var $finalPic= importPic("<img src='img/tree-2.jpg' alt=''>", "<img src='img/tree-3.jpg' alt=''>");
document.write($finalPic, "<br>");

//return 문 : 함수 구문 내부에서 진행을 중단하고 더이상 진행하지 않도록 함수 구문에서 빠져나오는 역할(반복문에서 break와 동일)
...
function treeImg(){
	...
	$n++;  //$n= 1, 2, 3, 4, 5, 6
	if($n>=$arr_tree.length){  //$n=6 도달했을 때 
		return;
	}
	treeImg();
}
...
//지역변수는 함수구문 내부에서 선엉ㄴ하여 함수 내부에서만 영향을 주는 변수(지역변수의 값을 외부로 전달하기 위해서는 반드시 return 참조 변수가 필요)
var $num_01 = 400; //전역변수
function varNum(){
	var $num_01=600;  //지역변수
