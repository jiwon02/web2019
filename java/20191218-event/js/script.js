...
var $realWidth = window.innerWidth;  //모니터 화면이 아닌 실제 브라우저의 가로값
console.log("브라우저 가로폭 : "+$realWidth);
var $realHeight=window.innerHeight  //모니터 화면이 아닌 실제 브라우저의 세로값
...
/*
function resizeEvt(){
	var $realWidth = window.innerWidth;  //모니터 화면이 아닌 실제 브라우저의 사이즈
...
function scrollEvt(){
	...
	console.log($scrollY);  //브라우저 상단으로부터 세로축으로 얼마나 떨어져 있는가를 값을 가져옴 (스크롤바의 상단 기준)
	if($scrollY>=72){
...
scrollEvt();  //최초 브라우저 로딩시 함수문을 로드하여 현재 스크롤 위치를 파악하고 해당하는 항목을 실행해라
