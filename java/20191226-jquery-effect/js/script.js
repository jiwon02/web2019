$(document).ready(function(){
	$(".showBtn").click(function(){
		...
		//$(".ef_01").show("slow");  //1초만에 최종 가로값, 세로값으로 도달
		//$(".ef_01").show("fast");  
	...
	//toggle() 메서드 : display:block =>display:none로 변경 / display:none =>display:block로 변경
	$(".cont").click(function(){
	...
	ease-in : 최초 시간대비 속도가 느리게 적용되다가 마지막 도달 시점에서 빠르게 적용
	...
	*/
	...
	$(".ani_left").click(function(){
		$(".ef_04").stop().animate({"margin-left":$moveTo-=100}, 500)  //stop() 메서드 : 이전 메모리상에서 걸려 있는 반복적인 요소를 제거하고 1회만 실행하도록 선언
	});
	...
	/*
	$(".menu>li").hover(function(){  //마우스가 영역 내부로 진입
