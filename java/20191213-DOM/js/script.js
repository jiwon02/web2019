window.onload=function(){	//윈도우라는 최상위 객체가 로딩이 완료되면 function(함수문)을 실행해라

	var $firstObj = document.getElementById("title");
	console.log($firstObj);

	$firstObj.style.color="red"; //<h2 id="title" style="color:red;">문서객체 모델 테스트</h2>
	$firstObj.style.backgroundColor="#ffffaa"; //<h2 id="title" style="background-color:#ffffaa;">문서객체 모델 테스트</h2>
	$firstObj.innerHTML="<span>더보기</span>"; //<h2 id="title" style="background-color:#ffffaa;"><span>더보기</span></h2>
	$firstObj.innerHTML="<span>감추기</span>"; //<h2 id="title" style="background-color:#ffffaa;"><span>감추기</span></h2>
	...
	/*클래스명을 기준으로 선택자를 정할 때 getElementsByclassName("list")[인덱스번호]*/
	...
	console.log($mList);
	var	$mList_parent=$mList.parentNode; //parentNode는 선택자의 바로 상위 보모만을 선택햐여 반환(노드(엔터값 포함)는 무시됨)
	console.log($mList_parent);
	$mList_parent.style.backgroundColor="#aaffaa";

	var $mList_children = $mList_parent.children;  //children은 선택자의 하위 자식요소를 모두 반환(HTML 코딩 성의 엔터값이 무시. 실제 요소명만을 자식으로 저장)
	console.log($mList_children);
	$mList_children[3].style.listStyle="none";	
	var $mList_childrenNodes = $mList_parent.childNodes;  //childNodes는 선택자의 하위 요소를 모두 반환(HTML 코딩상의 엔터값까지 자식 요소로 저장. 엔터값에 의한 공백도 자식으로 저장)
	console.log($mList_childrenNodes);
	$mList_childrenNodes[3].style.fontWeight="bold";	
	
	var $f_child = $mList_parent.firstChild;  //노드라는 개념 적용상 첫번째 엔터값을 지정
	console.log($f_child);
	var $l_child = $mList_parent.lastChild;  //노드라는 개념 적용상 마지막 엔터값을 지정
	console.log($l_child);

	var $prevSibling= $mList_children[2].previousSibling.previousSibling;  //노드라는 개념상 엔터값을 지정하고 기 이전 노드인 <li>를 선택하여 반환(product)
