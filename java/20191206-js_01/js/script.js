...
/*증감 연산자 : 1씩 증가시키거나 1씩 감소시킬 때 사용하는 연산자*/
/*
...
/*논리연산자 ( ||연산자 => or 연산자 : 내부의 값이 하나라도 true면 전체의 값은 true )*/
var $a = 10;
var $b = 12;
var $c = 24;
var $d = 21;
...
/*삼항조건 연산자  조건문? 실행문1 : 실행문2   => 조건식이 true 이면 실행문 1번을 실행, 조건식이 false 라면 실행문 2번을 실행*/
var $hour = 8;
...
if($season>=0 && $season<=12){
	...
	alert("입력한 숫자가 유효하지 않습니다. 재입력 해 주세요.", "<br>");
	location.reload();
}
*/
...
document.write("<div class=' bx bx_"+$box_num+"' style='background:#ffaaff;'>"+$box_num+"</div>");}else{ //3의 배수가 아닌 숫자
...
document.write("<div class='clear')></div>");
document.write("<h3>for 문</h3>"); //#1초기값->#2조건식->#3실행문->#4증감식
for($i=1; $i<=10; $i++){
	document.write("<div class='for_bx'>"+$i+"</div>");
}

document.write("<div class='clear')></div>");
//50개의 박스를 구성. 3의 배수, 4의 배수, 3과 4의 교집합이 되는 배수의 박스 영역 색상을 교체
for(k=1; k<=50; k++){
	if(k%3==0 && k%4!=0){ //3의 배수이면서 4의 배수가 아닌 박스 영역
		document.write("<div class='for_bx2' style='background:#ffffaa'>"+k+"</div>");
	}else if(k%3!=0 && k%4==0){ //3의배수가 아니면서 4의 배수인 경우
...
for($p=1; $p<=12; $p++){
	if($p == 7){  //$p = 1, 2, ,3, 4, 5, 6 박스를 구성하고 7번 박스는 만들지 않는다. 그리고 그 다음에 적용될 8, 9, 10 , 11, 12까지 다시 구성
...
//실습 응용 2
var $in2=prompt("시작할 구구단 수를 입력하세요", "");
for($e=$in2;$e<=9; $e++){
...
var numString = 24;
console.log(typeof numString );  //nember(숫자형 데이터)
...
var rstNo1 = parseInt(strNo); //정수 형태의 숫자형 데이터로 변환
console.log(rstNo1);
console.log(typeof rstNo1);

var rstNo2 = parseFloat(strNo); //실수 형태의 숫자형 데이터로 변환
console.log(rstNo2);
console.log(typeof rstNo2);


//종합 실습
var number1 = Number(prompt("첫번째 수", ""));  //var rst_num1 = Number(number1);
