$(document).ready(function(){
	...
	//load() 이벤트 : 페이지가 로딩된 후에 이벤트를 발생시킨다. 외부파일을 호출하여 로드할 때 사용 가능(공통적인 요소인 header와 footer를 호출하여 화면상에 html문서 형태로 가져올 수 있음. 단, 호스팅 서버를 활용하는 것이 필수 항목)
	$(".click_ev").click(function(){
		$(".click_ev").text("click 완료");
	});
	$(".a_click").click(function(e){
		//e.preventDefault();
		$(".a_click").css("color", "#f00");
		//return false;
	});
	//href의 속성을 막는 방법
	//#1. 클릭이벤트를 종료하기 직전에 return false; 선언
	//#2. 매개변수(e)를 선언하고 바로 하단에 e.preventDefault(); 작성
	//#3. a 태그 내부에 href 속성값으로 javascript:void(0); 작성
	...
	*/
	//hover()이벤트 : 마우스가 영역 내부로 진입했을 때 이벤트 발생+마우스가 영역 밖으로 빠져나왓을 때 이벤트 발생
	$(".card_img").hover(function(){  //마우스가 영역 내부로 진입시 이벤트 발생 (mouseenter)
		$(".card_img").attr("src", "img/card.png");
	}, function(){  //마우스가 영역 밖으로 빠져나왔을 때 이벤트 발생 (mouseleave)
		$(".card_img").attr("src", "img/ace.png");
	});
	...
	$(document).mousemove(function(e){
		var x_position=e.pageX; //브라우저 기준의 마우스 위치의 x좌표
		var y_position=e.pageY; //브라우저 기준의 마우스 위치의 y좌표
		console.log("현재 마우스의 x좌표 : "+x_position);
		console.log("현재 마우스의 y좌표 : "+y_position);
	});
*/
	$("#user_id").focus(); //브라우저가 모두 로딩되는 순간 해당하는 입력박스에 포커스를 강제 실행
